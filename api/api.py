...
class AssessmentResource(ModelResource):
	...
	def get_pending_assessment(self, request, **kwargs):
		...
		last_assessment_push = AssessmentPush.objects.filter(user__id=request.user.id).order_by("-sent")[:1][0]
		...
		if(last_assessment_push.exists()):
			...
			last_assessment = Assessment.objects.filter(id=last_assessment_push.assessment_id)[:1][0]
